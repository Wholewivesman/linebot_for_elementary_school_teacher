...
def test_start(event, qid, user_id):
    ...
    if db.
        line_api.reply_message(event.reply_token, TextMessage(text='發生錯誤，題目不存在'))
    else:
        #玩家狀態更改
...
def test_end(user_id):
    ...
    message.append(TemplateSendMessage(
                        alt_text='測驗結束，是否查看測驗結果',
                        template=ConfirmTemplate(
                            title='查看分數',
                            text='測驗結束，是否查看測驗結果',
                            actions=[                              
                                PostbackTemplateAction(label='查看分數',text='*查看分數*',data='cscore+1'),
                                PostbackTemplateAction(label='不查看分數',text='*不查看分數*',data='cscore+0')
                            ]
                        )
                    )
                )
    ...
    line_api.reply_message(event.reply_token, 
                           TemplateSendMessage(
                                ...
                                template=ButtonsTemplate(
                                    ...
                                    thumbnail_image_url='https://imgur.com/WSIqNjO.jpg',
                                    actions=[
                                            #data為內部回傳資訊，使用者看不到
    ...
    line_api.reply_message(event.reply_token, 
                           TemplateSendMessage(
                                ...
                                template=ButtonsTemplate(
                                    title=f'第{qid}題',
                                    text=f'{test.question}',
                                    thumbnail_image_url='https://imgur.com/YoCuhnE.jpg',
                                    actions=[
                                            #data為內部回傳資訊，使用者看不到
                                            PostbackTemplateAction(label=f'(A){test.opA}', text='*已選擇(A)*', data=f"a{qid}+A+{ANS}"),
                                            PostbackTemplateAction(label=f'(B){test.opB}', text='*已選擇(B)*', data=f"a{qid}+B+{ANS}"),
